...
router.post('/register', [
  ...
  [
    ...
    body('role')
      .isIn(['admin', 'parent', 'teacher'])
      .withMessage('Role must be admin, parent, or teacher'),
    body('phone')
      .optional()
      .matches(/^[\+]?[1-9][\d]{0,15}$/)
  ...
  try {
    ...
    const { email, password, firstName, lastName, role, phone, children, subjects, qualifications, experience, hourlyRate, availability } = req.body;
...
router.put('/me', [
  ...
  try {
    ...
    if (req.user.role === 'parent' && req.body.children) {
      updateFields.children = req.body.children;
    }

    if (req.user.role === 'teacher') {
      if (req.body.subjects) updateFields.subjects = req.body.subjects;
      if (req.body.qualifications) updateFields.qualifications = req.body.qualifications;
